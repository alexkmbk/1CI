...
действия. Обработка может быть внешней, тогда, на неё в сохраняется
...
время выполнения конкретной задачи. Одним из важнейших параметров,
...
Задача может быть запущена в ручную, из командной строки или по
расписанию.
...
- Обработка для запуска встроенного в платформу механизма тестирования
(параметр запуска ****CheckModules****);

- Обработка для перепроведения списка документов конфигурации;

- Обработка для формирования списка отчетов конфигурации.
...
ShowMessages &lt;Булево&gt; - определяет необходимость отображать
интерактивные сообщения пользователю (помимо записи в журнал).
...
На текущий момент реализовано две обработки:
...
Выполнение задач с помощью HTTP-сервиса
]===================================
...
OneCI, то строка http-запроса к задаче с кодом 2 хранилища с кодом 1
